function [solution,model] = maxGrowthOnPS(model,plot_flag,print_flag)
...
model = changeRxnBounds(model,'rxn00062',3.437,'b');
...
if solution.f > 0
    [~,bio_idx] = intersect(model.rxns,'bio_DvH_33gam');
    ...
    if plot_flag
        plotSRFlux(model,solution,0.05);
...
perl('./SVG_flux_adjust_test4.pl','drawing_moreW25A_new.svg', 'test_out.txt', 'PS_solution.svg');
end