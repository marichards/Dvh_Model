...
model = changeRxnBounds(model,'rxn00062',3.3,'b');
...
model = changeRxnBounds(model,'rxn14407',0,'u');
...
% Alter biomass to the proper GAM value, which is 5
model = setModelGAM(model,'HS');
...
if solution.f > 0
    [~,bio_idx] = intersect(model.rxns,'bio_DvH_2gam');
...
perl('./SVG_flux_adjust_test4.pl','drawing_moreW25A_new.svg', 'test_out.txt', 'HS_solution.svg');
end