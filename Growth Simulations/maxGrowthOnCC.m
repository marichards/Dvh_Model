function [solution,model] = maxGrowthOnCC(model,plot_flag,print_flag)
...
% Lactate
model = changeRxnBounds(model,'EX_cpd00159(e)',-8.9,'b');
% Sulfate
model = changeRxnBounds(model,'EX_cpd00048(e)',-1,'l');
...
model = changeRxnBounds(model,'rxn00062',3.0813,'b');
...
% Alter biomass to the proper GAM value, which is 115
model = setModelGAM(model,'CC');
...
function plotCCFlux(model,solution,threshhold)
...
perl('./SVG_flux_adjust_test4.pl','drawing_moreW25A_CC.svg', 'test_out.txt', 'CC_solution.svg');
end